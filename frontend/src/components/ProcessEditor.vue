        ...
        <ElTooltip placement="top" effect="light">
          <template #content>
            <div class="v2-tooltip">
              <p class="tooltip-title">V2 Alpha版本</p>
              <ul class="tooltip-list">
                <li>两阶段生成,避免截断问题</li>
                <li>多线程并发处理,提升性能</li>
                <li>自动检测并修复重复项</li>
              </ul>
              <p class="tooltip-note">适用于功能过程较多(≥10个)的场景</p>
            </div>
          </template>
          <ElButton
            type="warning"
...
/**
 * 提交全部过程，校验必填项后通知父组件 (V2 Alpha版本)
 */
function confirmProcessesV2() {
  const emptyProcesses = processes.value.filter(
    (process) => !process.description?.trim(),
  );
  ...
  updateSortIndexes();
  emit('confirm-v2', processes.value);
}
