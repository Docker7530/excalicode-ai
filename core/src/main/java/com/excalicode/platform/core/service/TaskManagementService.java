...
public class TaskManagementService {
    ...
    public List<TaskDraftResponse> parseTaskTemplate(MultipartFile file) {
        ...
        try (InputStream inputStream = file.getInputStream(); Workbook workbook = WorkbookFactory.create(inputStream)) {
            ...
            for (int rowIndex = 1; rowIndex <= lastRowNum; rowIndex++) {
                ...
                if (!StringUtils.hasText(title) && !StringUtils.hasText(description) && !StringUtils.hasText(workloadRaw)) {
                    continue;
                }
    ...
    public TaskBatchDetailResponse publishTaskBatch(TaskPublishRequest request, SysUser creator) {
        ...
        Set<Long> assigneeIds = request.getTasks().stream()
        ...
        for (TaskPublishRequest.TaskPublishItem item : request.getTasks()) {
            ...
            if (item.getWorkloadManDay() == null || item.getWorkloadManDay().compareTo(BigDecimal.ZERO) <= 0) {
        ...
        batch.setDescription(StringUtils.hasText(request.getBatchDescription())
                ? request.getBatchDescription().trim() : null);
    ...
    public List<TaskBatchSummaryResponse> listAllBatches() {
        List<ProjectTaskBatch> batches = projectTaskBatchService.lambdaQuery()
                .orderByDesc(ProjectTaskBatch::getPublishedTime)
                .list();
        if (CollectionUtils.isEmpty(batches)) {
            return List.of();
        }
        Map<Long, List<ProjectTask>> taskMap = loadTasksByBatch(batches.stream()
                .map(ProjectTaskBatch::getId)
                .toList());
        Map<Long, SysUser> userMap = loadUsers(resolveUserIdsFromBatches(batches, taskMap));
        return batches.stream()
                .map(batch -> buildBatchSummary(batch, taskMap.getOrDefault(batch.getId(), List.of()), userMap))
    ...
    public List<TaskBatchSummaryResponse> listBatchesForAssignee(Long assigneeId) {
        ...
        List<ProjectTaskBatch> batches = projectTaskBatchService.lambdaQuery()
                ...
                .orderByDesc(ProjectTaskBatch::getPublishedTime)
                .list();
        Map<Long, List<ProjectTask>> taskMap = tasks.stream().collect(Collectors.groupingBy(ProjectTask::getBatchId));
        Map<Long, SysUser> userMap = loadUsers(resolveUserIdsFromBatches(batches, taskMap));
        return batches.stream()
                .map(batch -> buildUserBatchSummary(batch, taskMap.getOrDefault(batch.getId(), List.of()), assigneeId, userMap))
                .collect(Collectors.toList());
    }
    ...
    public TaskResponse updateTaskAssignee(Long taskId, Long assigneeId) {
        ...
        Map<Long, SysUser> userMap = loadUsers(resolveUserIdsFromTasks(List.of(task), task.getCreatedBy()));
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    public TaskResponse updateTaskStatus(Long taskId, TaskStatus status, SysUser operator, boolean allowOverride) {
        ProjectTask task = projectTaskService.getById(taskId);
        if (task == null) {
            throw new BusinessException("任务不存在");
        }
    ...
    private Set<Long> resolveUserIdsFromBatches(List<ProjectTaskBatch> batches, Map<Long, List<ProjectTask>> taskMap) {
    ...
    private Map<Long, List<ProjectTask>> loadTasksByBatch(List<Long> batchIds) {
        ...
        List<ProjectTask> tasks = projectTaskService.lambdaQuery()
                .in(ProjectTask::getBatchId, batchIds)
                .list();
        return tasks.stream().collect(Collectors.groupingBy(ProjectTask::getBatchId));
    }

    private TaskBatchSummaryResponse buildBatchSummary(ProjectTaskBatch batch,
    ...
    private TaskBatchSummaryResponse buildUserBatchSummary(ProjectTaskBatch batch,
                                                           List<ProjectTask> tasks,
                                                           Long assigneeId,
                                                           Map<Long, SysUser> userMap) {
        List<ProjectTask> myTasks = tasks.stream()
        ...
        response.setCompletedTasks((int) myTasks.stream()
                .filter(task -> TaskStatus.COMPLETED.name().equalsIgnoreCase(task.getStatus()))
                .count());
    ...
    private TaskBatchDetailResponse buildBatchDetailResponse(ProjectTaskBatch batch,
                                                             List<ProjectTask> tasks,
                                                             Map<Long, SysUser> userMap) {
