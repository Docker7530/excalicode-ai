...
- **Frontend**：Vue 3 + Composition API、Vite 7、Element Plus、Vue Router、Axios、Vitest、oxlint、oxfmt、Sass、unplugin-auto-import / Components。
...
- **Frontend**：oxfmt（80 列、2 空格、单引号、分号、Trailing comma）统一格式；oxlint 作为主 Lint，禁止未使用变量，组件名使用 PascalCase，模板事件/属性统一连字符；API 调用通过 `src/api/request.js` 统一封装，返回 Promise，禁止直接在组件里写裸 Axios。
